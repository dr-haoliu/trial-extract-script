...
for ids in final_nct_list:
    ...
    continuing = False
    for j in res1:
        if j[5] is None or j[6] is None or j[7] is None:
            continuing = True
        study_type += str(j[0])
        brief_title += str(j[1])
        ...
        phase += str(j[4])
        start_date = str(j[5])
        primary_completion_date = str(j[6])
        completion_date = str(j[7])
    ...
    if len(res3) == 0:
        data = tuple([last_id, ids, study_type, brief_title, official_title, overall_status, phase, str(start_date),
    ...
    #print(data)
    cursor2.execute("INSERT INTO " +table+ " VALUES " + str(data))
    # To insert records into our local table one by one, uncomment the following line
    connection2.commit()
    cursor2.execute("Select * from "+table+" where nct_id='" + ids + "'")
    result = cursor2.fetchall()
    print(result)

# To do a batch commit or a batch insertion of records into the table, uncomment the following line
# connection2.commit()
